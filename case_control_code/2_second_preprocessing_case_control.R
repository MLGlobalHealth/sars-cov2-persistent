...
# Finding the final controls for the study and adding Charlson comorbidity index values ----------------------
...
subset_cases_and_controls_all_with_metadata_geo_diag <- readRDS(file="")
...
# Limiting up to 10 controls per case --------------

process_controls <- function(df) {
  # Step 1: For each event_id (which links cases with controls)
  df_cases_controls_limited <- df %>%
    group_by(event_id) %>%
    # Separate cases and controls, keep all cases
    group_modify(~ {
      cases <- .x %>% filter(type == "case") # Extract the case
      controls <- .x %>% filter(type == "control") # Extract the controls
      # Step 2: Randomly sample up to 10 controls, or keep all if less than 10
      controls_limited <- if(nrow(controls) > 10) {
        controls %>% sample_n(10)
      } else {
        controls
      }
      # Step 3: Combine cases and selected controls back together
      bind_rows(cases, controls_limited)
    }) %>%
    ungroup()
  
  # Step 4: Summarize the number of controls per case
  control_summary <- df_cases_controls_limited %>%
    group_by(event_id) %>%
    summarise(num_controls = sum(type == "control")) %>%
    summarise(
      avg_controls = mean(num_controls),                        # Mean number of controls per case
      p25_controls = quantile(num_controls, 0.25),              # 25th percentile
      median_controls = quantile(num_controls, 0.50),           # Median (50th percentile)
      p75_controls = quantile(num_controls, 0.75),              # 75th percentile
      IQR_controls = IQR(num_controls),
      minimum = min(num_controls),
      maximum = max(num_controls)
    )
  
  # Return the dataframe with limited controls and the control summary
  list(df_cases_controls_limited = df_cases_controls_limited, control_summary = control_summary)
}

# Apply the function to both dataframes
result_geo <- process_controls(subset_cases_and_controls_all_with_metadata_geo_diag)

subset_cases_and_controls_all_with_metadata_geo_diag_limited <- result_geo$df_cases_controls_limited
summary_geo <- result_geo$control_summary
